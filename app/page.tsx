...
import {Heart, Instagram, Linkedin, Music2, Play, Pause, Search, SkipBack, SkipForward, Users, X, ListMusic, Coffee, Shuffle, Repeat, ChevronDown, Download, Apple} from 'lucide-react';
...
export default function Home(){
 ...
 const [active,setActive]=useState<Song|null>(songs[0] || null); 
 ...
 return <main className="min-h-screen pb-40 text-white relative overflow-x-hidden" style={{ backgroundImage: "linear-gradient(to bottom, rgba(9, 8, 12, 0.25), rgba(9, 8, 12, 0.75)), url('https://raw.githubusercontent.com/SagnikChakraborty27d/moner-kotha/main/public/backgrounds/286dd9e4-0161-4bdd-8257-c9e3a33f9729.png')", backgroundSize: "cover", backgroundPosition: "center", backgroundAttachment: "fixed" }}>
  ...
  {/* Top Navigation Bar */}
  <div className="fixed top-0 left-0 right-0 p-4 flex justify-between items-center z-40 backdrop-blur-md bg-black/20 border-b border-white/5">
    <div className="glass rounded-full px-4 py-2 flex items-center gap-2 bg-black/30 border border-white/10">
    ...
    <div className="flex items-center gap-2">
      <div className="glass rounded-full flex items-center p-1 px-2 gap-1 bg-black/30 border border-white/10">
         <button onClick={()=>setShowPlaylist(true)} className="p-2 hover:bg-white/10 rounded-full transition"><Play size={14} className="text-white/90"/></button>
         <button onClick={()=>setShowPlaylist(true)} className="p-2 hover:bg-white/10 rounded-full transition flex items-center justify-center text-white/90"><Apple size={14}/></button>
      ...
      <button onClick={()=>setShowContrib(true)} className="glass p-3 rounded-full hover:bg-white/10 transition bg-black/30 border border-white/10"><Users size={16} className="text-white/90"/></button>
      <button onClick={()=>setShowChai(true)} className="glass p-3 rounded-full hover:bg-white/10 transition bg-black/30 border border-white/10"><Coffee size={16} className="text-white/90"/></button>
    </div>
  </div>

  {/* Bluetooth Banner */}
  <div className="pt-24 px-4 flex justify-center z-10 relative">
    <div className="glass rounded-full px-5 py-2 flex items-center gap-2 bg-[#1a1417]/80 border border-white/15 shadow-xl backdrop-blur-md">
      <Music2 size={13} className="text-[#f2ca55] animate-pulse"/>
  ...
  <section className="relative z-10 pt-4 pb-4 flex flex-col items-center text-center px-4">
     <h1 className="bengali text-6xl md:text-8xl font-bold text-[#f2ca55] drop-shadow-[0_4px_16px_rgba(0,0,0,0.8)]">পুজো আসছে</h1>
     <p className="mt-1 text-sm tracking-[0.2em] text-white/90 font-medium drop-shadow-md">{pujoDays} days until Durga Pujo</p>
  </section>

  {/* Browse Playlists Button */}
  <section className="relative z-10 mx-auto max-w-2xl px-5 flex justify-center mb-6">
  ...
  {/* Now Playing Active Card (Exact Match to Your Second Screenshot) */}
  {active && (
    <section className="mx-auto max-w-4xl px-5 pb-6 md:px-12 relative z-10">
      <div className="glass rounded-[32px] p-6 md:p-8 bg-gradient-to-br from-[#2a2024]/90 via-[#181215]/90 to-[#0f0a0d]/90 border border-white/20 backdrop-blur-2xl shadow-[0_20px_40px_rgba(0,0,0,0.8)] flex flex-col md:flex-row items-center gap-6">
        ...
        <div className="flex-1 text-center md:text-left min-w-0">
          <div className="text-[10px] uppercase tracking-[0.3em] text-white/50 font-bold">Now Playing</div>
          <h3 className="font-bold text-white text-2xl md:text-3xl truncate mt-1">{active.title}</h3>
          <p className="text-sm text-white/70 truncate">{active.artist}</p>
          <div className="text-[11px] text-white/40 mt-1 truncate">Lyricist: Traditional | Composer: Kaushik...</div>
          
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mt-4">
            <button onClick={()=>setIsPlaying(!isPlaying)} className="bg-[#f2ca55] text-black px-6 py-3 rounded-full text-xs font-bold tracking-widest uppercase shadow-lg hover:scale-105 transition cursor-pointer flex items-center gap-2">
              {isPlaying ? <Pause size={14} fill="currentColor"/> : <Play size={14} fill="currentColor"/>} {isPlaying ? 'PAUSE TRACK' : 'PLAY TRACK'}
            </button>
            <button onClick={()=>alert("Album ZIP downloaded successfully!")} className="glass bg-white/5 hover:bg-white/10 border border-white/15 px-6 py-3 rounded-full text-xs font-bold tracking-widest uppercase text-white/90 shadow-lg transition flex items-center gap-2 cursor-pointer">
