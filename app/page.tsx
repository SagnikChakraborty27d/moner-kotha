...
import { Heart, Instagram, Linkedin, Music2, Play, Pause, Search, SkipBack, SkipForward, Users, X, ListMusic, Coffee, Shuffle, Repeat, ChevronDown, Volume2 } from 'lucide-react';
...
export default function Home() {
  ...
  const filtered = useMemo(() => songs.filter(s => ((s.title + ' ' + s.artist + ' ' + s.category).toLowerCase().includes(q.toLowerCase())) && (!mood || s.moods?.includes(mood))), [q, mood]);
  ...
  return (
    <main className="min-h-screen pb-40 text-white selection:bg-[#f2ca55]/30" style={{ backgroundImage: "linear-gradient(to bottom, rgba(9, 8, 12, 0.4), rgba(9, 8, 12, 0.85)), url('/backgrounds/286dd9e4-0161-4bdd-8257-c9e3a33f9729.png')", backgroundSize: "cover", backgroundPosition: "center", backgroundAttachment: "fixed" }}>
      ...
      <AnimatePresence>
        {showSplash && (
          <motion.div initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.8 }} className="fixed inset-0 z-[100] bg-[#09080c] flex flex-col items-center justify-center p-6 text-center">
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 1 }} className="w-36 h-36 md:w-48 md:h-48 rounded-full overflow-hidden shadow-[0_0_30px_rgba(242,202,85,0.3)] border-2 border-[#f2ca55]/40 mb-6 bg-black/40 flex items-center justify-center">
              <span className="bengali text-4xl text-[#f2ca55]">মনের কথা</span>
            </motion.div>
      ...
      <div className="fixed top-0 left-0 right-0 p-4 flex justify-between items-center z-40 backdrop-blur-md bg-black/10 border-b border-white/5">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowContrib(true)} className="glass p-3 rounded-full hover:bg-white/10 transition bg-black/20 border border-white/10"><Users size={16} className="text-white/80" /></button>
          <button onClick={() => setShowChai(true)} className="glass p-3 rounded-full hover:bg-white/10 transition bg-black/20 border border-white/10"><Coffee size={16} className="text-white/80" /></button>
        </div>
      </div>

      {/* Hero Pujo Asche Section */}
      <section className="relative z-10 pt-36 pb-6 flex flex-col items-center text-center px-4">
         <h1 className="bengali text-6xl md:text-8xl font-bold text-[#f2ca55] drop-shadow-[0_4px_16px_rgba(0,0,0,0.6)]">পুজো আসছে</h1>
      ...
      <section className="relative z-10 mx-auto max-w-2xl px-5 flex justify-center mb-10">
        <button onClick={() => setShowPlaylist(true)} className="glass bg-[#1a1417]/70 hover:bg-[#1a1417] border border-white/15 rounded-full px-6 py-3.5 flex items-center gap-3 text-xs tracking-widest text-white/90 font-semibold uppercase shadow-2xl transition-all cursor-pointer backdrop-blur-md">
      ...
      <section className="mx-auto max-w-4xl px-5 pt-2 md:px-12">
        <div className="glass rounded-[30px] p-5 md:p-7 bg-black/30 border border-white/10 backdrop-blur-xl shadow-xl">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-2 rounded-full border border-white/10 bg-black/30 px-4 py-2.5 md:w-80">
          ...
          <div className="scrollbar mt-5 flex gap-3 overflow-x-auto pb-1">
            {moods.map(([icon, label]) => (
              <button key={label} onClick={() => setMood(mood === label ? '' : label)} className={`whitespace-nowrap rounded-full border px-4 py-2 text-sm transition ${mood === label ? 'border-[#f2ca55]/60 bg-[#f2ca55]/20 text-[#f2ca55]' : 'border-white/10 bg-white/5 text-white/70 hover:bg-white/10'}`}>
                {icon} {label}
              </button>
            ))}
          </div>
      ...
      <section className="mx-auto max-w-4xl px-5 pt-8 md:px-12">
        <div className="glass relative overflow-hidden rounded-[34px] p-7 md:p-10 bg-black/30 border border-white/10 backdrop-blur-xl shadow-xl">
      ...
      <footer className="mx-auto flex max-w-4xl flex-col gap-4 px-5 pb-10 pt-14 text-sm text-white/40 md:flex-row md:items-center md:justify-between md:px-12">
        ...
        <button onClick={() => setShowContrib(true)} className="hover:text-white transition text-left">About the creator</button>
      </footer>

      {/* Modals & Popups */}
      <AnimatePresence>
        {showPlaylist && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] bg-black/70 backdrop-blur-md flex flex-col justify-end sm:grid sm:place-items-center p-0 sm:p-4">
        ...
        {showChai && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[80] bg-black/80 backdrop-blur-md grid place-items-center p-5">
            <motion.div initial={{ y: 30, scale: .97 }} animate={{ y: 0, scale: 1 }} exit={{ y: 30, scale: .97 }} className="glass relative w-full max-w-sm rounded-[34px] p-8 text-center bg-[#1a1417]/90 border border-white/15 backdrop-blur-2xl">
              ...
              <div className="mt-6 bg-white p-3 rounded-2xl inline-block shadow-2xl">
                 <img src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=upi://pay?pa=9366745761@fam" alt="UPI QR Code" className="w-48 h-48 rounded-xl opacity-90" />
              </div>
      ...
      {/* Hidden React Player for Real Audio Playback */}
      {active && (
      ...
      {active && (
        <motion.div initial={{ y: 100 }} animate={{ y: 0 }} className="fixed bottom-4 left-4 right-4 z-[60] max-w-2xl mx-auto">
            ...
            <div className="flex items-center gap-4">
               ...
               <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10px] text-white/40 mt-1 flex items-center gap-1">
                    <Volume2 size={10} className="text-[#f2ca55]" /> Playing audio via YouTube Engine • {active.duration}
                  </div>
               </div>
